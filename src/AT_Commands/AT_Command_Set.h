	...
	class AT_Command_Set {
		...
		public:
			...
			bool GMI(uint8_t & _Manufacturer) {
				...
				// Get Char
				for (size_t i = 0; i < Buffer.Read_Order - 2; i++) _Modem_Manufacturer[i] = Buffer_Variable[i];
			...
			bool GMM(uint8_t & _Model) {
				...
				// Get Char
				for (size_t i = 0; i < Buffer.Read_Order - 2; i++) _Modem_Model[i] = Buffer_Variable[i];
				
				// Handle Manufacturer
				if (strcmp(_Modem_Model, "GE910-QUAD") == 0) _Model = 1;
			...
			bool MONIZIP(uint16_t & _Operator, char * _LAC, char * _Cell_ID, uint8_t & _dBM) {
				...
				// Handle Variables
				uint8_t _Variable_Count = sscanf(Buffer_Variable, "#MONIZIP: %05d,%02d,%01d,%04c,%04c,%03d,-%03d,%01d", &_Operator, &Operator.BSIC, &Operator.QUAL, _LAC, _Cell_ID, &Operator.ARFCN, &_dBM, &Operator.TIMADV);
